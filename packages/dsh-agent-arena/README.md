...
dsh plugin --profile web add ../../dist/dsh-agent-arena-1.0.1.tgz
```

Restart the existing DSH Web process and refresh its page. The selected profile must provide `agents`, `subprocess`, `storageDomain`, Typert Remotes, Settings, and at least two usable provider/model routes. See the suite [installation guide](../../INSTALL.md).
