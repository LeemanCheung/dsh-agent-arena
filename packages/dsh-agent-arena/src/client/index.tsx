...
import { DEFAULT_VALIDATION_COMMANDS, type ArenaConfig, type ArenaMatch, type ArenaStatus, type Contestant, type ContestantResult, type ValidationCommand } from '../models.ts'
...
const initialValidation: ValidationCommand[] = [...DEFAULT_VALIDATION_COMMANDS]
...
function ArenaPanel({ api }: { api: Api }): React.ReactElement {
  ...
  const ready = Boolean(repository.trim() && objective.trim() && contestants.every(item => item.provider.trim() && item.model.trim()) && validation.length > 0 && validation.every(item => !problem(item.command)))
  ...
  return React.createElement('section', { className: styles.arena, 'aria-labelledby': 'arena-title' },
    ...
    React.createElement('div', { className: styles.grid },
      React.createElement('form', { className: styles.card, onSubmit: (event: React.FormEvent<HTMLFormElement>) => { event.preventDefault(); start() } },
        ...
        React.createElement('p', { className: styles.preflight }, React.createElement('strong', null, '请添加项目验证：'), 'Arena 不预填会固定成功的通用命令。请加入项目自身的测试、类型检查或构建命令；仅接受空格分隔 argv，如 corepack pnpm test。'),
